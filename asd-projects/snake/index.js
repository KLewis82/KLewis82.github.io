...
function hasCollidedWithApple() {
  ...
  */
  ...
  snake.head.row; // the current row of snake.head
  snake.head.column; // the current column of snake.head
  if (apple.column == snake.head.column && apple.row == snake.head.row ){
    return true
  }  else{
    return false;
  }
...
function handleAppleCollision() {
  ...
  */
  ...
  console.log("srow", snake.tail.row, "scol", snake.tail.column);
...
function hasCollidedWithSnake() {
  ...
  */
  var headSnakeSquare = snake.body[0];
  for (var i = 1; i <= snake.body.length - 1; i++){
    var snakeSquare = snake.body[i];
    if (headSnakeSquare.row === snakeSquare.row && headSnakeSquare.column === snakeSquare.column){
      return true;
    } 
  }
  return false;
